...
def dict_to_obs(space, obs_dict):
    ...
    :param space: (stable_baselines.common.spaces.Space) an observation space.
    ...
    if isinstance(space, spaces.Dict):
        return obs_dict
    elif isinstance(space, spaces.MixedMultiDiscreteBox):
...
def obs_space_info(obs_space):
    """
    Get dict-structured information about a Space.

    Dict spaces are represented directly by their dict of subspaces.
    Tuple spaces are converted into a dict with keys indexing into the tuple.
    Unstructured spaces are represented by {None: obs_space}.

    :param obs_space: (stable_baselines.common.spaces.Space) an observation space
    ...
    elif isinstance(obs_space, spaces.MixedMultiDiscreteBox):
